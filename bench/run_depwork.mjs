#!/usr/bin/env node
/**
 * DeepDepCat Depwork benchmark runner (via ACP, work_mode=depwork).
 *
 * Usage:
 *   node bench/run_depwork.mjs --base http://127.0.0.1:31524 --out bench/results/depwork-run-YYYY-MM-DD
 *   node bench/run_depwork.mjs --task literature-review
 *
 * Prereqs: app running with ACP enabled, DeepSeek key configured,
 * permissions bypassed for the run (permission_mode=bypass is passed to
 * session/new so unattended depwork tools can write outputs).
 */
...
const WORK_DIR = path.join(HERE, "depwork-work");
const TASKS_FILE = path.join(HERE, "tasks", "depwork.md");
  ...
  try {
    while (true) {
      ...
      while ((sep = buffer.indexOf("\n\n")) >= 0) {
        ...
        if (event === "prompt/update" && payload.promptId === promptId &&
    ...
    if (terminal) return { terminal, transcript };
  }
...
async function runTask(base, task, outDir) {
  ...
  const session = await rpc(base, "session/new", {
    ...
    system_prompt:
      "You are running a Depwork benchmark task. Follow the task brief exactly, " +
      "meet every acceptance point, cite sources where required, and keep " +
      "outputs inside the workspace. When done, give a concise completion " +
      "report listing which acceptance points were met.",
  });
  ...
  let evidence = null;
  try {
    evidence = await rpc(base, "session/evidence", { session_id: session.sessionId });
  } catch {
    evidence = null;
  }
  ...
  const record = {
    ...
    evidence_counts: evidence
      ? { events: evidence.events?.length ?? 0, messages: evidence.messages?.length ?? 0 }
      : null,
    failure: null,
  };
  await writeFile(path.join(outDir, `${task.id}.json`), JSON.stringify(record, null, 2), "utf8");
  if (evidence) {
    await writeFile(
...
async function main() {
  ...
  const outDir = args.out ?? path.join(HERE, "results", `depwork-${new Date().toISOString().slice(0, 10)}`);
  await mkdir(outDir, { recursive: true });
  console.log(`Running ${tasks.length} depwork tasks against ${args.base}`);
