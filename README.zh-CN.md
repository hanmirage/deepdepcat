...
> 本仓库是 DeepDepCat 桌面应用的开源之家，采用 Apache-2.0 协议发布。同时也是发布渠道与社区中心。欢迎任何问题、功能建议与反馈。
...
| 每会话终端 | 每个聊天会话一个持久交互式 shell，切换会话即切换终端，支持 python/vim/htop |
...
| 信任与安全 | 3 级权限模式（只读 / 接受编辑 / 完全放行）、沙箱执行、bash AST 安全分析、行内确认 |
| 静默更新 | 小版本退出时自动安装零打扰；大版本走更新按钮 |

## 从源码构建

DeepDepCat 已完全开源（Apache-2.0）。自行构建桌面应用：

1. 安装 [Rust](https://rustup.rs)、[Node.js 18+](https://nodejs.org) 与 [Tauri 2 前置依赖](https://v2.tauri.app/start/prerequisites/)。
2. 运行 `npm install`。
3. 运行 `npm run tauri build`（开发调试用 `npm run tauri dev`）。

源码结构：[src](./src) — React 前端 · [src-tauri](./src-tauri) — Rust 后端 · [server](./server) — FastAPI 更新/认证/遥测后端 · [depwork-mcp](./depwork-mcp) — WPS Office MCP 服务器。
